...
ping localhost -n 1 >nul
if not exist control.bat echo [31m[CRITICAL STOP][0m YOU HAVE A HOLE IN YOUR CONTROL PANEL!!! &&set "criterror=3" &&goto critstop
echo [0;0H
echo                                                                                                                .
echo                                                                                                                .
echo                                                                                                                .
...
ping localhost -n 1 >nul
if not exist act.bat echo [31m[CRITICAL ERROR][0m APP EXECUTER MISSING! &&set "criterror=4" &&goto critstop
cls
call mdl.bat
call apl.bat
mnlogon.bat
goto main
if not %resolution%==1 if not %resolution%==2 echo [31m[ERROR][0m Invalid resolution setting. &&ping localhost -n 3 >nul &&set criterror=5 &&goto critstop
;
:critstop
ping localhost -n 1 >nul
if not exist STP (
...
if %criterror%==4 (
echo Micunymos Vista has now shut down because of a fatal error.
echo This error indicates the App Executer is missing.
echo To fix this, try searching for a replacement, or reinstall Micunymos Vista.
echo Additional info:
echo Stop 0x04, [no parameters]
pause
)
if %criterror%==5 (
echo Micunymos Vista has now shut down because of a fatal error.
echo This error indicates the resolution setting is outside of the supported range.
echo To fix this, try setting it again in the "options" file, or reinstall Micunymos Vista.
echo Additional info:
